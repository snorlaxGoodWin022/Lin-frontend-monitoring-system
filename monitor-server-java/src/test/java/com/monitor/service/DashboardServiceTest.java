...
class DashboardServiceTest {
    ...
    void getPerformanceTrend() {
        AggregationResults<Map<String, Object>> mockResults = new AggregationResults<>(List.of(), new Document());
        when(mongoTemplate.aggregate(any(Aggregation.class), eq("performances"), eq(Map.class)))
                .thenReturn((AggregationResults) mockResults);

        List<Map<String, Object>> result = dashboardService.getPerformanceTrend("app1", 24);
    ...
    void getErrorDistribution() {
        ...
        AggregationResults<Map<String, Object>> mockResults = new AggregationResults<>(List.of(errorStat), new Document());
        when(mongoTemplate.aggregate(any(Aggregation.class), eq("errors"), eq(Map.class)))
                .thenReturn((AggregationResults) mockResults);

        List<Map<String, Object>> result = dashboardService.getErrorDistribution("app1", 24);
    ...
    void getPvUv() {
        AggregationResults<Map<String, Object>> emptyResults = new AggregationResults<>(List.of(), new Document());
        when(mongoTemplate.aggregate(any(Aggregation.class), eq("behaviors"), eq(Map.class)))
                .thenReturn((AggregationResults) emptyResults);

        Map<String, List<Map<String, Object>>> result = dashboardService.getPvUv("app1", 24);
